...
function scaffoldGoProject({ force = false } = {}) {
  ...
  // go.mod for SSR root module
  const rootGoModPath = path.join(goRoot, 'go.mod');
  const rootGoMod = `module server-ssr

go 1.25.0

replace signals-ssr v0.0.0 => ./ssr/

require signals-ssr v0.0.0
`;
  const rootW1 = writeFileIfMissing(rootGoModPath, rootGoMod, { force });
...
`;
  ...
  const wrote = [w1, rootW1, w2, w3, w4, w5, w6].some(Boolean);
  if (wrote) {
...
// Returns a map: identifier -> { varName, dataUrl }ssr
...
function emitGo(name, parts, values, srcPath, sf, propsInfo, assetMap = {}, initials = undefined) {
    ...
    code.push(`func ${name}WithProps(props ${ctx.propsType}) ssr.VNode {`);
...
async function run() {
  ...
  const dir = await opendir(path.join(repoRoot, 'dist/assets'));
  for await (const entry of dir) {
      fs.copyFileSync(path.join(repoRoot, 'dist/assets', entry.name), path.join(assetsDir, entry.name));
      console.log('Copied from', path.join(repoRoot, 'dist/assets', entry.name), 'to', path.join(assetsDir, entry.name));
  }
}
